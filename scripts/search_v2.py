...
def run_experiments():
    ...
    for lr, dropout, batch_size, epochs, main_w, aux_w in itertools.product(
        ...
        try:
            ...
            """
            hate_test = LatentHatredDataset(f"{args.dataset_dir}/latent_hatred_3class_test.csv", split="test") # Adjusted filename
            stereo_test = StereoSetDataset(f"{args.dataset_dir}/stereoset_test.csv", split="test") # Adjusted filename
            sarcasm_test = ISarcasmDataset(f"{args.dataset_dir}/isarcasm_test.csv", split="test") # Adjusted filename
            implicit_fine_test = ImplicitFineHateDataset(f"{args.dataset_dir}/implicit_fine_labels_test.csv", split="test") # Adjusted filename

            dataloaders_test = {
                "main": DataLoader(hate_test, batch_size=args.batch_size),
                "stereo": DataLoader(stereo_test, batch_size=args.batch_size),
                "sarcasm": DataLoader(sarcasm_test, batch_size=args.batch_size),
                "implicit_fine": DataLoader(implicit_fine_test, batch_size=args.batch_size)
            }

            metrics = evaluate(model, dataloaders_test, device=device)
            ...
            results.append({
                ...
                "avg_f1": avg_f1,
                "main_precision": metrics["main"]["precision"],
                "main_recall": metrics["main"]["recall"],
            })
