...
class FinalModelTrainer:
    ...
    def train_final_model(self):
        ...
        dropout = self.best_config["dropout"]
        batch_size = self.best_config["batch_size"] 
        weight_decay = self.best_config["weight_decay"]
        # Use consistent parameters from best_config
        #dropout = self.best_config.get("dropout", 0.1)
        #batch_size = self.best_config.get("batch_size", 32)
        ...
        model = SingleTaskBERT(dropout=dropout).to(device)
        optimizer = AdamW(
            model.parameters(),
...
def main():
    ...
    elif all(getattr(args, param) is not None for param in ['lr']):
